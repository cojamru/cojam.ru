...
const createPopupLink = (link = '', text = '', isExternal = false) => {
	...
	return $create.elem('li', $create.link(link, text, '', linkOptions));
};

const generateLikely = ({
	container,
	options = { title: '', URL: '', heading: '', image: '' },
}) => {
	...
	if (options.title) {
		let _heading = options.heading ? `${options.heading} "${options.title}"` : options.title;
...
const showPopup = ({ content, options = { heading: '', id: '', title: '', image: '' } }) => {
	...
	document.body.insertBefore(popup, $make.qs('.container'));
};
...
export const $parser = {
	music: ({ data = {}, container }) => {
		...
		let generatePopup = ({ album = {}, heading = '' }) => {
			...
			let albumArtist = album.artist ? album.artist : 'Cojam';

			albumPopupDes.appendChild(
				$create.elem(
					'p',
			...
			if (album.description) {
				albumPopupDes.appendChild(
					$create.elem('p', `<b>Описание</b>: ${album.description.replace(/\n/g, '<br>')}`),
				);
			}
			...
			if (album.embed && album.embed.type) {
				...
				let _user = album.embed.user ? album.embed.user : 'team@cojam.ru';

				switch (album.embed.type) {
					case 'bc':
						albumEmbedSrc += `bandcamp.com/EmbeddedPlayer/album=${albumEmbedSrcID}/size=large/bgcol=ffffff/linkcol=${color}/artwork=none/transparent=true`;
						break;
					case 'sc':
						albumEmbedSrc += `w.soundcloud.com/player/?url=${https}api.soundcloud.com/playlists/${albumEmbedSrcID}&color=${color}`;
						break;
					case 'ym':
						albumEmbedSrc += `music.yandex.ru/iframe/#playlist/${_user}/${albumEmbedSrcID}/hide/cover/title/`;
						break;
				}
			...
			if (album.links && Object.keys(album.links).length > 0) {
				...
				if (album.links.bc) {
					albumEmbedLinks.appendChild(
						createPopupLink(
							`${https}${album.links.bc.user}.bandcamp.com/album/${album.links.bc.album}`,
							'🎵 BandCamp',
							true,
						),
					);
				...
				if (album.links.vk) {
					albumEmbedLinks.appendChild(
						createPopupLink(
							`${https}vk.com/${album.links.vk.com}?z=audio_playlist${album.links.vk.ID}`,
							'🎼 ВКонтакте',
							true,
						),
					);
				...
				if (album.links.yt) {
					albumEmbedLinks.appendChild(
						createPopupLink(
							`${https}www.youtube.com/playlist?list=${album.links.yt}`,
							'📼 YouTube',
							true,
						),
					);
		...
		let parseAlbum = (album = {}) => {
			let albumContainer = $create.elem('div', '', 'music__album'),
				albumAbout = $create.elem('div', '', 'music__album--about'),
				albumCover = $create.elem('img', '', 'music__album--cover');

			let albumID = album.id ? album.id : album.title.toLowerCase().replace(' ', '-');
	...
	games: ({ data = {}, container }) => {
		...
		let generatePopup = ({ game = {}, heading = '' }) => {
			...
			switch (game.platform) {
				case 'flash':
					gamePopupPlatform += 'Flash';
					break;
				case 'pc':
					gamePopupPlatform += 'PC';
					break;
				case 'online':
					gamePopupPlatform += 'браузерная игра';
					break;
				case 'gba':
					gamePopupPlatform += 'Game Boy Advance';
					break;
				default:
			...
			if (game.contest && game.contest.name) {
				let contest =
					game.contest.link && game.contest.link != ''
			...
			if (game.links && Object.keys(game.links).length > 0) {
				if (game.links.site) {
					gamePopupLinks.appendChild(createPopupLink(game.links.site, '🏠 Сайт игры', true));
				}
				...
				if (game.links.itch) {
					gamePopupLinks.appendChild(createPopupLink(game.links.itch, '🕹️ Itch.io', true));
				}

				if (game.links.dl) {
					gamePopupLinks.appendChild(createPopupLink(game.links.dl, '☁️ Скачать', true));
				}
		...
		let parseGame = (game = {}) => {
			let gameContainer = $create.elem('div', '', 'games__game'),
			...
			let gameID = game.id ? game.id : game.title.toLowerCase().replace(' ', '-');
